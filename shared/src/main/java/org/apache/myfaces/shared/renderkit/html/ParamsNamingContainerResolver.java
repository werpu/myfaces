...
public class ParamsNamingContainerResolver
{
    ...
    public static boolean isPostBack(FacesContext context)
    {
        ...
        return hasViewState(context, requestParameterMap.keySet());
    }
    ...
    public static String resolveNamingContainerPrefix(FacesContext facesContext)
    {
        ...
        if(viewRoot == null)
        {
            ...
            String firstViewStateKey = resolveViewStateKey(facesContext, reqParamMap.keySet());
    ...
    private static boolean hasViewState(FacesContext context, Set<String> keys)
    {
        if(keys.contains(ResponseStateManager.VIEW_STATE_PARAM))
        {
            return true;
        }
        ...
        while(keyIt.hasNext())
        {
            if(keyIt.next().contains(searchPattern))
            {
                return true;
            }
        }
    ...
    private static String resolveViewStateKey(FacesContext context, Set<String> keys)
    {
        if(keys.contains(ResponseStateManager.VIEW_STATE_PARAM))
        {
        ...
        while(keyIt.hasNext())
        {
            ...
            if(key.equals(ResponseStateManager.VIEW_STATE_PARAM)  || key.contains(searchPattern))
            {
                return key;
            }
        }
